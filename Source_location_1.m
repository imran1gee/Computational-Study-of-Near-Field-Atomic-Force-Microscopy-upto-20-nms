...
ratio = 1/(Dme*Dmu);
for i = 1:1:length(z0)
    ETh(i) = Et*(exp(1i*kt*sin(theta)*z0(i))+((1-ratio)/(1+ratio))*exp(-1i*kt*sin(theta)*z0(i))); % Terahertz source field
    ...
    for k2 = 1:1:length(t2)
        ai2= ai2+1*exp(-1i*sqrt((-zl+2*z0(i))^2)*sin(t2(k2)))...
    ...
    R(i) = sqrt(z0(i)^2);
    I(i) = ETh(i)/(1+alpha(i));  % Selfconsistent current, Isc = It+Isc*alpha
end

%%
for i = 1:1:length(z0)
    ETha(i) = Et*(exp(1i*kt*sin(theta)*z0(i))-exp(-1i*kt*sin(theta)*z0(i)));% Terahertz source field
    alphaa(i) = -(1/4)*(besselh(0,2,0.01)-besselh(0,2,abs(2*z0(i)))); % The proportionality weight for self-consistent current
    Ia(i) = ETha(i)/(1+alphaa(i));  % Selfconsistent field,
end
...
save('Data_Source_1.mat','R',"I","Ia")

figure('DefaultAxesFontSize',24)

% 
plot((1/(2*pi))*R,real(I),'-ob',(1/(2*pi))*R,real(Ia),'-or','LineWidth',0.3)
% ylim([-100,100])
